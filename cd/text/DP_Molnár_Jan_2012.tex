...
\abstractCS{Diplomová práce se zabývá doménou informací potřebných pro studium na Fakultě informačních technologií ČVUT a přístupem studentů k těmto informacím. Cílem práce je vytvořit ontologii reprezentující danou doménu a~demonstrovat její užití na mobilní aplikaci sloužící jako průvodce studenta. Dosaženo toho je serverovou aplikací implementovanou na \textit{Node.js} shromažďující informace a poskytující je prostřednictvím \textit{SPARQL} \textit{endpointu} mobilní aplikaci implementované v \textit{HTML5}. Práce se zaměřuje na analýzu, návrh, realizaci i~testování vzniklých aplikací. % Důraz byl kladen na 
...
\abstractEN{The master's thesis deals with the domain of information needed for studying at CTU, Faculty of Information Technology and the students' access to this information. The goal of the work is to create an ontology representing the given domain and demonstrate its usage on a mobile application used as a student's guide. The goal is achieved by a server information gathering application implemented on \textit{Node.js} which provides this information through a \textit{SPARQL} endpoint to a mobile application implemented in \textit{HTML5}. The thesis is focused on an analysis, design, implementation and testing of the created applications.}
